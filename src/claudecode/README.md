...
| globalConfigHome | Absolute host path whose .claude/ sub-directory and .claude.json are linked into the container's ~/.claude/ and ~/.claude.json. Defaults to the host user's home directory (~). Must be inside the host home directory. | string | - |
| projectConfigFolder | Absolute host path whose .claude/ sub-directory and .claude.json are linked into ${workspaceFolder}/.claude/ and ${workspaceFolder}/.claude.json. Leave empty to skip project-level config. Must be inside the host home directory. | string | - |
